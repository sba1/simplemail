...
STATIC ASM SAVEDS struct address_match_entry *matchentry_construct(REG(a0, struct Hook *h), REG(a2, Object *obj),REG(a1,struct NList_ConstructMessage *msg))
{
	...
	if ((new_entry->is_group = entry->is_group))
	{
...
{
	struct address_match_entry *entry = (struct address_match_entry *)msg->entry;
	if (entry->is_group) addressbook_free_group(entry->o.group);
	else addressbook_free_entry_new(entry->o.entry);
	free(entry);
}
...
/********************************************
 Boopsi Dispatcher
*********************************************/
STATIC BOOPSI_DISPATCHER(ULONG,AddressMatchList_Dispatcher,cl,obj,msg)
{
	switch(msg->MethodID)
	{
