...
int app_init(void)
{
	...
	App = ApplicationObject,
		...
		MUIA_Application_Copyright,		"Copyright (c) 2000-2010 by Sebastian Bauer & Hynek Schlawack",
			...
			else
			{
				...
				if (cs != user.config.default_codeset && !lock)
				{
					...
					rc = sm_request(NULL,_("The charset configured for SimpleMail (%s) doesn't match\n"
													...
													user.config.default_codeset?_("Unknown"):user.config.default_codeset->name,
													cs->name);
